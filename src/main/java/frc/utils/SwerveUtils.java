...
public class SwerveUtils {
	 ...
	 * @param target The target value the algorithm will step towards.
	 ...
	 * the specified step towards the specified target.
	...
	public static double stepTowardsCircular(double current, double target, double stepsize) {
		...
		if (difference <= stepsize) {
			...
			if (current + 2 * Math.PI - target < stepsize || target + 2
				* Math.PI - current < stepsize) {
				return target;
			} else {
	...
	public static double wrapAngle(double angle) {
		...
		if (angle == twoPi) {
		// Handle this case separately to avoid floating point
		// errorswith the floor after the division in the case below
			return 0.0;
		} else if (angle > twoPi) {
